...
# --- Подготовка Данных и Прогнозирование (ОСАДКИ) ---
...
def main():
    ...
    # Заполнение пропусков и расчет месячной суммы осадков
    full_index = pd.date_range(start=start, end=end, freq='D')
    data = data.reindex(full_index)
    data['prcp_filled'] = data['prcp'].fillna(0) # для осадков пропуск часто означает отсутствие
    
    # Resample к месяцам (сумма осадков за месяц)
    monthly_prcp = data['prcp_filled'].resample('ME').sum()

    # Разделяем на train (1993-2022) и test (2023)
    train_series = monthly_prcp[monthly_prcp.index.year < 2023]
    test_series = monthly_prcp[monthly_prcp.index.year == 2023]
    ...
    print("Calculating Fourier coefficients and forecasting precipitation...")
    new_coefs = get_new_fourier_coefs(matrix, p)
    test_pred = predict_next_period(new_coefs, l)
    
    # Осадки не могут быть отрицательными, обнулим артефакты прогноза если они < 0
    test_pred = [max(0, val) for val in test_pred]
    
    test_pred_series = pd.Series(test_pred, index=test_series.index)

    # Оценка
    mae = round(mean_absolute_error(test_series, test_pred_series), 2)
    # MAPE для осадков может выдавать деление на 0 если факт = 0, поэтому посчитаем аккуратно
    try:
        mape = round(mean_absolute_percentage_error(test_series, test_pred_series), 3)
    except:
        mape = "N/A (дел. на ноль)"
    ...
    plt.title('Прогноз суммарных осадков за месяц в Магадане на 2023 год', fontsize=15)
    plt.xlabel('Месяц', fontsize=14)
    plt.ylabel('Сумма осадков (мм)', fontsize=14)
    plt.grid(True)
    
    # Добавляем текст с ошибками
    plt.text(test_series.index[2], test_series.max() * 0.9, f'MAE = {mae} мм', fontsize=15)
    if isinstance(mape, (int, float)):
        plt.text(test_series.index[2], test_series.max() * 0.82, f'MAPE = {mape}', fontsize=15)
    
    plt.tight_layout()
    plt.savefig('magadan_fourier_prcp_forecast.png')
    print("Plot saved as 'magadan_fourier_prcp_forecast.png'")
